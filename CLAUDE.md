...
- **snakemake engine** (`src/recastatlas/engines/snakemake.py`): local and docker backends. Resolves `toplevel` itself (local dir, or `github:org/repo@ref:subpath` via `git clone` into the instance workdir), writes `initdata` as a configfile, and runs the `snakemake` CLI as a subprocess with `--directory <workdir>` so outputs land where results extraction expects. On the docker backend, the whole snakemake run happens inside the `recast/recastatlas` image (which ships snakemake+apptainer): `prepare_run(materialize=True)` copies local toplevels into the workdir so a single `$PWD` mount suffices, and `--privileged` is added automatically when `RECAST_SNAKEMAKE_SDM` includes apptainer — that's how per-rule `container:` directives work on macOS, where apptainer doesn't exist natively. `submit` still rejects snakemake entries; `validate_entry` branches to a `--dry-run` check. `testing/snakecontainertest` is the apptainer-path smoke entry.
...
- `docker/Dockerfile` (published by `.github/workflows/docker.yml` as `recast/recastatlas`, runtime default pinned in `config.py` via `.bumpversion.cfg`) contains both the yadage stack and snakemake+apptainer; its base is the Alpine `docker:*-cli` image, so package names/availability follow Alpine.
